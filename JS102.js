...
function over25(anotherArray){
  for (var i = 0; i < anotherArray.length; i++){
    if (anotherArray[i].value > 25) {
      newArray += anotherArray[i];
      return newArray;
    }
  }
}
...
*/
var newerArray = [];
var inputArray = $(".inputb").val();
$(document).on("click", ".bttn", function(){
  newerArray += inputArray.split(',');
  return newerArray;
})

var result = over25(newerArray);

sorts(result);

for (var i = 0; i >= sorted; i++) {
  $(".something").append('<div class="number">' + returnToPage[i] +', </div>')
};
